...
#
# Refer to spyce.py
# CVS: $Id: spyceCmd.py 726 2005-05-13 16:29:50Z jbe $
##################################################
...
def spyceMain(cgimode=0, cgiscript=None, 
  ...
  if not configFile:
    configFile = spycePreload.defaultConfigFile()
      ...
      try:
        ...
        if compileonlyMode:
          ...
          if spyceUtil.isTagCollection(f):
            code, coderefs, modrefs = \
              spyceCompile.spyceCompile(f.read(), script, '', server, True)
            output.write(code)
          else:
