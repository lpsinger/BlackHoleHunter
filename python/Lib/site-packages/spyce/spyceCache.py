...
#
# Refer to spyce.py
# CVS: $Id: spyceCache.py 765 2005-05-31 16:39:04Z jbe $
##################################################

import spyceLock, spyceUtil
import os, string
from md5 import md5
...
class fileCache(cache):
  ...
  def _filename(self, key):
    return os.path.join(self._cachedir, self._encodeKey(key))
  def __getitem__(self, key):
    f = None
    try:
      try:
        f = open(self._filename(key), 'rb')
        return loads(f.read())
      finally:
        if f: f.close()
    except IOError: pass
  ...
  def _encodeKey(self, key):
    return 'spyceCache-' + md5(str(key)).hexdigest()
...
class semanticCache(cache):
  ...
  def __init__(self, cache, valid, generate, lock=None):
    """ lock needs to accept spyceLock.hashLock semantics for lock/unlock """
    self.valid = valid
    self.generate = generate
    self.cache = cache
    if lock is None:
      lock = spyceLock.dummyLock()
    self.lock = lock
  def get(self, key):
